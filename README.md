...
API**. LoremOpus is not a functional assistant.
...
uv sync --locked                           # installs LoremOpus into .venv
```
...
forward additional arguments to their respective CLIs. Each starts LoremOpus on
...
the local LoremOpus API**, run
`COPILOT_OFFLINE=false ./scripts/run-copilot.sh -p "Say hello"`. This is no
longer an offline CLI session.

The launchers set `LOREMOPUS_TOOLS=0` by default. To opt into demo Bash tool
calls, use `LOREMOPUS_TOOLS=1 ./scripts/run-claude.sh` (or `run-copilot.sh`).
...
The default wire model ID is `claude-opus-4-8-loremopus`. Request processing is
...
loremopus.py              Generated prose, thinking, and demo commands
...
scripts/_loremopus-server.sh  Shared launcher lifecycle
